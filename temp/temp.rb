...
OptionParser.new do |opts|
  ...
  opts.on("--audio_music_volume_adjustment=[DECIBELS]", Float, "Audio volume adjustment (in decibels) (default: #{options.audio_music_volume_adjustment})") do |volume_adjustment|
    options.audio_music_volume_adjustment = volume_adjustment
  end
...
#Audio
volume_adjustment_db = -5
filter_chains += ["[#{slides.count+1}:a]volume=#{volume_adjustment_db}dB[adjusted_music]"]
...
cmd = [
  ...
  *options.audio_narration ? ["-i", options.audio_narration] : [],
  *options.audio_music ? ["-i", options.audio_music] : [],
