...
OptionParser.new do |opts|
  ...
  opts.on("--narration-volume-adjustment VALUE", Float, "Narration volume adjustment in decibels") do |value|
    options[:narration_volume_adjustment] = value
  end
...
# #   '-filter_complex', "[0:v]trim=duration=#{durations.sum},setpts=PTS-STARTPTS[v0]; [1:a]atrim=duration=#{durations.sum},asetpts=PTS-STARTPTS,volume=-#{options[:music_volume_adjustment]}dB:eval=frame[a1]; [2:a]atrim=duration=#{durations.sum},asetpts=PTS-STARTPTS,volume=-#{options[:narration_volume_adjustment]}dB:eval=frame[a2]; [a1][a2]amix=inputs=2:duration=first:dropout_transition=2[a]",
#   '-filter_complex', "[0:v]trim=duration=#{durations.sum},setpts=PTS-STARTPTS[v0]; [1:a]atrim=duration=#{durations.sum},asetpts=PTS-STARTPTS,volume=1.0dB:eval=frame[a1]; [2:a]atrim=duration=#{durations.sum},asetpts=PTS-STARTPTS,volume=-1.0dB:eval=frame[a2]; [a1][a2]amix=inputs=2:duration=first:dropout_transition=2[a]",
#
#     '-map', '[v0]',
#     '-map', '[a]',
#     '-y',
...
ffmpeg_command = [
    ...
    '-i', options[:background_music],
    '-i', options[:narration_audio],
  '-filter_complex', "[1:a]volume=1.0[narr]; [2:a]volume=2.0[bg]; [narr][bg]amix=inputs=2:duration=longest[aout]",
    '-map', '0:v',
    '-map', '[aout]',
    '-y',
    '-c:v', 'copy', '-c:a', 'aac',
  ...
  '-filter_complex', "[0:v]trim=duration=#{durations.sum},setpts=PTS-STARTPTS[v0]; [0:a]atrim=duration=#{durations.sum},asetpts=PTS-STARTPTS[orig_audio]; [1:a]volume=-10dB:eval=frame,asplit=#{num_transitions}#{first_line_str};#{transition_effects_str}; [orig_audio]#{transition_outputs_str}amix=inputs=#{num_transitions + 1}:duration=first:dropout_transition=2[a]",
  '-map', '[v0]',
  '-map', '[a]',
  '-y',
  '-c:v', 'libx264',
